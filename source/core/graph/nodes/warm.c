...
static sp_ps_output_t stub_exec(spn_invocation_t* invocation, spn_dag_env_t* env, sp_mem_t mem) {
  ...
  ps.io.out.mode = SP_PS_IO_MODE_CREATE;
  ...
  sp_ps_output_t output = sp_ps_output(&child);
  sp_ps_free(&child);
  return output;
}

static s32 run_stub(spn_build_unit_t* build, const spn_zig_stub_t* stub, sp_str_t name, sp_str_t dir, spn_dag_env_t* env, sp_mem_t mem) {
  ...
  sp_ps_output_t run = stub_exec(&invocation, env, mem);
...
s32 spn_warm_stub_run(spn_build_unit_t* build, const spn_zig_stub_t* stub, sp_str_t name, spn_path_t stamp, spn_path_t output, spn_dag_env_t* env) {
  ...
  if (!sp_fs_exists(spn_path_str(&spn.roots, mem, stamp))) {
    rc = run_stub(build, stub, name, sp_fs_parent_path(out), env, mem);
  }
